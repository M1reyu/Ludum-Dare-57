...
const shopItem = GlobalVars.shopBuyables
var shopCalc = GlobalVars.ShopCalc.new()

var tntScene = preload("res://Scenes/Misc/TnT.tscn")
var oreScene = preload("res://Scenes/Player/ore_pop_up.tscn")
...
func _process(delta: float) -> void:	
	...
	if (Input.is_action_just_pressed("UseTnT")):
		if bombCount > 0 && tntTimeout <= 0:
			tntTimeout = GlobalVars.cooldowns.get(shopItem.Bomb)
			bombCount -= 1
		...
		if scannerBought && scanTimeout <= 0:
			scanTimeout = GlobalVars.cooldowns.get(shopItem.Scanner)
